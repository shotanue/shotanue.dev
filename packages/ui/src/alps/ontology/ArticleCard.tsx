...
export const ArticleCard: React.FC<ArticleCardProps> = ({ article, index, onClick }) => {
    ...
    return (
        <motion.a
            ...
            className="block p-6 rounded-lg border border-zinc-800/50 bg-zinc-900/30 backdrop-blur-sm transition-colors group cursor-pointer relative overflow-hidden"
            onClick={(e) => {
                if (onClick) {
                    e.preventDefault();
                    onClick();
        ...
        >
            ...
            <h3 className="text-xl font-bold text-zinc-100 mb-4 pr-16 leading-snug group-hover:text-blue-400 transition-colors">
                {article.title}
            </h3>
            ...
            {isExternal && (
                <div className="flex items-center gap-2 text-sm text-blue-400 mt-4 pt-4 border-t border-zinc-800/50">
                    ...
                    <span className="opacity-70 hover:opacity-100 transition-opacity break-all">
